...
- npm 10 or later
- Python 3.11 or later (managed via `uv` on Windows)

### Windows Setup Instructions

1. **Install `uv`** (a fast Python package and virtual environment manager):
   ```powershell
...
4. **Set up npm and start the project**:
   ```powershell
   npm install
...
The content launcher checks `PYTHON`, then common Python commands. When the virtual environment is active, it will automatically use that environment's Python.
